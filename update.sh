...
require_repo_files() {
    ...
    for name in "$@"; do
        status=$(${curl_bin} -sIL --retry 3 --connect-timeout 15 -o /dev/null -w '%{http_code}' "https://raw.githubusercontent.com/islamsc1/3x-ui/${ref}/${name}")
        if [[ "${status}" != "200" ]]; then
...
update_x-ui() {
    ...
    rm -f "${archive}.sha256"
    sidecar_code=$(${curl_bin} -sL --retry 3 --retry-delay 3 --connect-timeout 15 --max-time 60 -o "${archive}.sha256" -w '%{http_code}' "https://github.com/islamsc1/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz.sha256" 2> /dev/null)
    ...
    ${curl_bin} -fLRo "${xui_script_temp}" "https://raw.githubusercontent.com/islamsc1/3x-ui/${script_ref}/x-ui.sh" > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        ${curl_bin} -fLRo "${xui_rc_temp}" "https://raw.githubusercontent.com/islamsc1/3x-ui/${script_ref}/x-ui.rc" > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
