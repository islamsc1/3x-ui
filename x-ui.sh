...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/islamsc1/3x-ui/v${ver}-allowinsecure/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/islamsc1/3x-ui/v${ver}-allowinsecure/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using with-allow-insecure${plain}" >&2
        echo "https://raw.githubusercontent.com/islamsc1/3x-ui/with-allow-insecure/x-ui.sh"
    fi
}
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/islamsc1/3x-ui/with-allow-insecure/install.sh)${plain}"
    echo ""
